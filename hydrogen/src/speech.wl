(* ========================================================
   hydrogen/src/speech.wl — Spoken introduction synthesis

   Same three-tier fallback pattern established in images/src/speech.wl,
   thermo/src/speech.wl, and montecarlo/src/speech.wl (SpeechSynthesize[]
   -> platform-native TTS -> text-only STEMSay fallback). Duplicated
   rather than shared because apps do not import each other's src/ files
   -- only stem-core is shared.
   ======================================================== *)


(* ResampleLinear — general linear-interpolation resampler, any ratio. *)
ResampleLinear[data_List, rawSr_?NumericQ, targetSr_?NumericQ] :=
  Module[{n = Length[data], newN, ratio, oldPos, lo, frac},
    If[n < 2 || Abs[N[rawSr] - N[targetSr]] < 1.0, Return[data]];
    ratio = N[targetSr] / N[rawSr];
    newN  = Max[1, Round[n * ratio]];
    Table[
      oldPos = (i - 1) * (n - 1.0) / Max[newN - 1, 1];
      lo     = Clip[Floor[oldPos], {0, n - 2}];
      frac   = oldPos - lo;
      (1 - frac) * data[[lo + 1]] + frac * data[[lo + 2]],
      {i, newN}
    ]
  ];


(* HydrogenSpeakToBufferPlatform — OS-native TTS -> mono PCM at targetSr. *)
HydrogenSpeakToBufferPlatform[text_String, targetSr_Integer] :=
  Module[{id, wavPath, snd, data, rawSr},
    id      = ToString[RandomInteger[999999]];
    wavPath = FileNameJoin[{$TemporaryDirectory, "stem_hydrogen_say_" <> id <> ".wav"}];

    Switch[$OperatingSystem,

      "MacOSX",
        Module[{aiffPath, result, conv},
          aiffPath = FileNameJoin[{$TemporaryDirectory, "stem_hydrogen_say_" <> id <> ".aiff"}];
          result = Quiet[RunProcess[{"say", "-o", aiffPath, text}]];
          If[!AssociationQ[result] || result["ExitCode"] =!= 0 || !FileExistsQ[aiffPath],
            Quiet[DeleteFile /@ Select[{aiffPath, wavPath}, FileExistsQ]];
            Return[{}]];
          conv = Quiet[RunProcess[{"afconvert", aiffPath, wavPath, "-d", "LEI16", "-f", "WAVE"}]];
          Quiet[DeleteFile[aiffPath]];
          If[!AssociationQ[conv] || conv["ExitCode"] =!= 0 || !FileExistsQ[wavPath],
            Quiet[DeleteFile /@ Select[{wavPath}, FileExistsQ]];
            Return[{}]]],

      "Unix",
        Module[{result},
          result = Quiet[RunProcess[{"espeak-ng", "-w", wavPath, text}]];
          If[!AssociationQ[result] || result["ExitCode"] =!= 0 || !FileExistsQ[wavPath],
            result = Quiet[RunProcess[{"espeak", "-w", wavPath, text}]]];
          If[!AssociationQ[result] || result["ExitCode"] =!= 0 || !FileExistsQ[wavPath],
            Quiet[DeleteFile /@ Select[{wavPath}, FileExistsQ]];
            Return[{}]]],

      "Windows",
        Module[{psCmd, result},
          psCmd = "Add-Type -AssemblyName System.Speech; " <>
                  "$s = New-Object System.Speech.Synthesis.SpeechSynthesizer; " <>
                  "$s.SetOutputToWaveFile('" <>
                  StringReplace[wavPath, "\\" -> "/"] <> "'); " <>
                  "$s.Speak('" <> StringReplace[text, "'" -> "''"] <> "'); " <>
                  "$s.SetOutputToDefaultAudioDevice()";
          result = Quiet[RunProcess[{"powershell", "-NoProfile", "-Command", psCmd}]];
          If[!AssociationQ[result] || result["ExitCode"] =!= 0 || !FileExistsQ[wavPath],
            Quiet[DeleteFile /@ Select[{wavPath}, FileExistsQ]];
            Return[{}]]],

      _,
        Return[{}]
    ];

    snd = Quiet[Import[wavPath]];
    If[!AudioQ[snd], Quiet[DeleteFile[wavPath]]; Return[{}]];
    data  = Quiet[Flatten[N[AudioData[snd]]]];
    rawSr = Quiet[QuantityMagnitude[AudioSampleRate[snd]]];
    Quiet[DeleteFile[wavPath]];
    If[!ListQ[data] || Length[data] === 0 || !NumericQ[rawSr], Return[{}]];

    ResampleLinear[data, rawSr, targetSr]
  ];


(* BuildIntroBuffer — SpeechSynthesize[] -> platform TTS -> text-only.
   Used for both intros and (spectrum mode's) outro text. *)
BuildIntroBuffer[text_String, sr_Integer] :=
  Module[{buf, result, data, rawSr},

    buf = Quiet[Check[
      result = SpeechSynthesize[text];
      If[AudioQ[result],
        data  = Quiet[Flatten[N[AudioData[result]]]];
        rawSr = Quiet[QuantityMagnitude[AudioSampleRate[result]]];
        If[ListQ[data] && Length[data] > 0 && NumericQ[rawSr],
          ResampleLinear[data, rawSr, sr],
          {}
        ],
        {}
      ],
      {}
    ]];
    If[ListQ[buf] && Length[buf] > 0, Return[buf]];

    buf = HydrogenSpeakToBufferPlatform[text, sr];
    If[ListQ[buf] && Length[buf] > 0, Return[buf]];

    Print["[WARNING] Speech synthesis unavailable (SpeechSynthesize[] and " <>
          "platform TTS both failed) -- spoken intro will be skipped. Text:"];
    STEMSay[text];
    {}
  ];


(* ── Intro / outro text builders, one per mode ───────────────────── *)

BuildOrbitalsIntroText[orbitalKey_String, n_Integer, l_Integer, m_Integer, gridSize_Integer] :=
  "Hydrogen atom " <> OrbitalDisplayName[orbitalKey] <> " orbital: quantum numbers n=" <>
  ToString[n] <> ", l=" <> ToString[l] <> ", m=" <> ToString[m] <> ". " <>
  ToString[gridSize] <> " by " <> ToString[gridSize] <>
  " cross-section in the x z plane, scanned in Hilbert curve order. The brightest regions " <>
  "-- highest electron probability -- produce the highest pitch. Nodes, where the wave " <>
  "function is exactly zero, produce silence.";

BuildSpectrumIntroText[nMax_Integer, nLines_Integer] :=
  "Hydrogen emission spectrum, transitions from n=2 to n=" <> ToString[nMax] <> ". " <>
  ToString[nLines] <> " spectral lines from Lyman through Paschen series. You will hear " <>
  "first a chord of all lines simultaneously, then a sweep from ultraviolet through visible " <>
  "to infrared. The four Balmer series lines -- the visible hydrogen spectrum -- are marked " <>
  "with a soft bell tone during the sweep.";

BuildSpectrumOutroText[] :=
  "That was the complete hydrogen spectrum -- Lyman ultraviolet, Balmer visible light, and " <>
  "Paschen infrared, all produced by the same single electron falling between energy levels.";

BuildTransitionsIntroText[nRealizations_Integer, nStart_Integer, lStart_Integer] :=
  "Hydrogen electron cascade, " <> ToString[nRealizations] <>
  " realisations starting from n=" <> ToString[nStart] <> ", l=" <> ToString[lStart] <>
  ". Each realisation is a different random path through the energy levels, following " <>
  "quantum selection rules. All paths end at the ground state -- the lowest, final note. " <>
  "The differences between realisations are quantum randomness.";
